...
- **Few-shot bias:** The analyze step is seeded with 3 worked examples covering inverted comparisons, off-by-one errors, and type confusion. This suppresses style false-positives but may bias the model toward those three bug shapes; novel bug categories outside those archetypes may be under-detected. The zero-shot baseline is preserved as `ANALYZE_PROMPT_BASELINE` and toggleable via `FEWSHOT=0` for fair A/B comparison.
